...
1. **VS Code URLs can expire if sandbox is killed** - When you expose VS Code with `sandbox.exposeVscode()`, the URL will hang or become inaccessible if the sandbox is destroyed (via `await using` pattern or explicit disposal). If you want persistent VS Code access, use `.close()` to disconnect from the sandbox without destroying it.

2. **Don't use `await using` if you need the sandbox to stay alive** - The `await using` pattern automatically destroys the sandbox when the scope ends. If you need the sandbox to persist (e.g., for user access to VS Code/SSH), use manual connection management:
   ```typescript
   // Create sandbox (will stay alive after function exits)
   const sandbox = await Sandbox.create();
   const sandboxId = sandbox.id;

   // Disconnect but keep sandbox running
   await sandbox.close();

   // Later, reconnect to the same sandbox
   const reconnected = await Sandbox.connect({ id: sandboxId });
   ```

3. **Deno Deploy app must be created before deploying a sandbox** - When using `sandbox.deno.deploy()`, you must create a Deno Deploy app first using the `Client` API. The deploy method expects an app UUID, not a slug:
   ```typescript
   ...
   // Create app first
   const app = await client.apps.create({ slug: "my-app" });

   // Then deploy sandbox to it using the app's UUID
   await using sandbox = await Sandbox.create();
   // ... build your app in the sandbox ...
   await sandbox.deno.deploy(app.id, { // Use app.id, not app.slug!
     options: {
       production: true,
