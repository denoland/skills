...
## Programmatic Deployment with @deno/sandbox

The `@deno/sandbox` SDK includes a `Client` class for programmatically managing Deno Deploy apps and deployments:

```typescript
import { Client } from "@deno/sandbox";

// Create a client (uses DENO_DEPLOY_TOKEN from env, or pass token explicitly)
const client = new Client(accessToken);

// Create a new app (slug must be ≤ 32 characters)
const app = await client.apps.create({
  slug: "my-app", // max 32 chars!
});
// Returns: { id: "uuid-...", slug: "my-app", created_at: "...", updated_at: "..." }
...
// Deploy from a sandbox to an app
await using sandbox = await Sandbox.create({ /* ... */ });
// ... build your app in the sandbox ...

// Deploy using the app's UUID (NOT the slug!)
const deployResult = await sandbox.deno.deploy(app.id, {
  options: {
    production: true,
    build: {
      entrypoint: "main.ts",
...
1. **App slugs have a 32 character limit** - `client.apps.create()` will fail if slug is longer. Use short names or hash long IDs.

2. **`sandbox.deno.deploy()` expects a UUID, not a slug** - You must pass `app.id` (the UUID), not `app.slug`.

3. **Apps must be created before deploying** - Unlike the CLI's `deno deploy create`, the sandbox deploy method requires an existing app. Create it first with `client.apps.create()`.

4. **Slug uniqueness is per organization** - If a slug already exists in your org, `apps.create()` will throw an error. Check existing apps first or use try/catch with a fallback to `apps.list()`.

5. **`--unstable-kv` flag needed for Deno KV** - When running production builds with `deno serve`, you must include `--unstable-kv` flag to access `Deno.openKv()`. Example: `deno serve --env --unstable-kv -A _fresh/server.js`
